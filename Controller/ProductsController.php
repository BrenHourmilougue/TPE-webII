<?php
    ...
    class ProductsController extends Helper{
        ...
        function Home($params = null){
            ...
            if($logeado){
                $user = $_SESSION['EMAIL'];
                $this->view->ShowHome($productLimit, $categorias, $pagination, $page, $user);
            }else{
                $this->view->ShowHome($productLimit, $categorias, $pagination, $page);
            }
        ...
        function EditProduct($params = null){
            ...
            if($logeado && $_SESSION['ADMIN'] == 1){
                ...
                $images = $this->imageModel->GetImagenByProduct($product_id);
                $this->view->ShowEditProduct($product, $categorias, $images);
            }else{
                $this->view->ShowLocation('login');
            }
        }
        ...
        function UpdateProduct($params = null){
            ...
            if($logeado && $_SESSION['ADMIN'] == 1){
                ...
                if (isset($_POST['edit_product']) && isset($_POST['edit_price']) && isset($_POST['edit_stock'])
                 && isset($_POST['edit_description']) && isset($_POST['select_categoria'])){
                    $product = $_POST['edit_product'];
                    $price = $_POST['edit_price'];
                    $stock = $_POST['edit_stock'];
                    $description = $_POST['edit_description'];
                    $categoria = $_POST['select_categoria'];
                    $marca = $_POST['input_marca'];
                    $this->model->UpdateProduct($product,$price,$stock,$description,$categoria,$marca,$product_id);
                    $fileTemp = $_FILES['input_file']['tmp_name'];
                    for($i=0; $i<count($_FILES['input_file']['tmp_name']); $i++){
                        $name = basename($_FILES["input_file"]["name"][$i]);
                        if($name){
        ...
        function FilterProductsByCategoria($params = null){
            if (isset($_POST['select_categoria'])) {
                $categoria_id = $_POST['select_categoria'];
                $products = $this->model->GetProductsByCategoria($categoria_id);
                $categorias = $this->categoriasModel->GetCategorias();
                $this->view->ShowSearch($products, $categorias, $user = null, $categoria_id);
            }
        }
        ...
        function ItemDetail($params = null){
            ...
            if($logeado){
                ...
                $this->view->ShowItemDetail($product, $categoria, $images, $stars, $user, $Iduser, $admin);
            }else{
                $this->view->ShowItemDetail($product, $categoria, $images);
            }
        }
        //BORRA UNA IMAGEN
        ...
        function SearchItem(){
            ...
            if($logeado){
                $user = $_SESSION['EMAIL'];
                $this->view->ShowSearch($products, $categorias, $user);
            } else{
                $this->view->ShowSearch($products, $categorias);
            }
        }
    }
?>